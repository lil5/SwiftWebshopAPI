...
#### Kubernetes
To deploy your application to your Kubernetes cluster, run `helm install --name myapp .` in the `/chart/SwiftRest` directory. You need to make sure you change the `repository` variable in your `chart/SwiftRest/values.yaml` file points to the docker image containing your runnable application.
